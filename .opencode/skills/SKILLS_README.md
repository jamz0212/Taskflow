...
## Memoria Persistente (.opencode/memory/)

SaaS Factory incluye un sistema de memoria persistente POR PROYECTO que reemplaza la auto-memory de Claude Code.

**Por que?** La auto-memory de Claude Code guarda notas en `~/.opencode/projects/` (local a tu maquina). Eso significa que no viaja con el repo, no es versionado, no es compartido con tu equipo, y Claude decide que guardar sin tu control.

**Como funciona:**
- `.opencode/memory/MEMORY.md` es el indice (max 200 lineas, se carga automaticamente)
...
**Activacion:** La primera vez que se usa el skill `memory-manager`, automaticamente deshabilita la auto-memory de Claude Code en `.opencode/settings.json` y crea la estructura de carpetas.
...
|---------|------|-----------|
| PRP Template | `.opencode/PRPs/prp-base.md` | Skill `prp` |
| AI Templates | `.opencode/skills/ai/references/` | Skill `ai` |
| Design Systems | `.opencode/design-systems/` | Directo (5 sistemas) |
...
# Opcion 2: Manual
mkdir .opencode/skills/mi-skill
...
| V3 | V4 |
|----|-----|
| `.opencode/commands/*.md` | `.opencode/skills/*/SKILL.md` |
| `.opencode/agents/*.md` | `.opencode/skills/*/SKILL.md` (user-invocable: false, context: fork) |
| `.opencode/prompts/*.md` | `.opencode/skills/*/SKILL.md` |
