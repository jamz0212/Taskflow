...
name: memory-manager
description: |
  Sistema de memoria persistente POR PROYECTO. Guarda conocimiento en .opencode/memory/ dentro del repo,
...
> La memoria vive en `.opencode/memory/` DENTRO del repo. Versionada en git. El usuario tiene control total.
...
Crear o actualizar `.opencode/settings.json` en la raiz del proyecto:
...
**Por que?** La auto-memory de Claude Code guarda notas en `~/.opencode/projects/` (local a la maquina).
Eso significa que:
...
```bash
mkdir -p .opencode/memory/{user,feedback,project,reference}
```

### Paso 3: Crear indice MEMORY.md

Crear `.opencode/memory/MEMORY.md` con el template base si no existe:
...
Informar: "Memoria persistente activada. Auto-memory de Claude Code deshabilitada. Tu proyecto ahora guarda conocimiento en `.opencode/memory/`, versionado con git."
...
### Como consultar
1. Leer `.opencode/memory/MEMORY.md` (el indice)
