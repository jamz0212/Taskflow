...
Este skill actualiza las herramientas de desarrollo (carpeta `.opencode/`) a la ultima version disponible.
...
### Paso 3: Reemplazar .opencode/

Elimina la carpeta `.opencode/` actual del proyecto y copia la nueva:

```bash
# En el directorio del proyecto actual
rm -rf .opencode/
cp -r [RUTA_REPO_SF]/saas-factory/.opencode/ .opencode/
```
...
Cambios aplicados:
- .opencode/skills/ (skills actualizados)
- .opencode/PRPs/ (templates PRP actualizados)
- .opencode/skills/ai/references/ (AI templates actualizados)
- .opencode/design-systems/ (sistemas de diseno actualizados)
