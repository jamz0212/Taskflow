...
## Memoria Persistente (.agent/memory/)

SaaS Factory incluye un sistema de memoria persistente POR PROYECTO que reemplaza la auto-memory de Claude Code.

**Por que?** La auto-memory de Claude Code guarda notas en `~/.agent/projects/` (local a tu maquina). Eso significa que no viaja con el repo, no es versionado, no es compartido con tu equipo, y Claude decide que guardar sin tu control.

**Como funciona:**
- `.agent/memory/MEMORY.md` es el indice (max 200 lineas, se carga automaticamente)
...
**Activacion:** La primera vez que se usa el skill `memory-manager`, automaticamente deshabilita la auto-memory de Claude Code en `.agent/settings.json` y crea la estructura de carpetas.
...
|---------|------|-----------|
| PRP Template | `.agent/PRPs/prp-base.md` | Skill `prp` |
| AI Templates | `.agent/skills/ai/references/` | Skill `ai` |
| Design Systems | `.agent/design-systems/` | Directo (5 sistemas) |
...
| V3 | V4 |
|----|-----|
| `.agent/commands/*.md` | `.agent/skills/*/SKILL.md` |
| `.agent/agents/*.md` | `.agent/skills/*/SKILL.md` (user-invocable: false, context: fork) |
| `.agent/prompts/*.md` | `.agent/skills/*/SKILL.md` |
