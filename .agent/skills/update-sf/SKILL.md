...
Este skill actualiza las herramientas de desarrollo (carpeta `.agent/`) a la ultima version disponible.
...
### Paso 3: Reemplazar .agent/

Elimina la carpeta `.agent/` actual del proyecto y copia la nueva:

```bash
# En el directorio del proyecto actual
rm -rf .agent/
cp -r [RUTA_REPO_SF]/saas-factory/.agent/ .agent/
```
...
- .agent/PRPs/ (templates PRP actualizados)
- .agent/skills/ai/references/ (AI templates actualizados)
- .agent/design-systems/ (sistemas de diseno actualizados)
