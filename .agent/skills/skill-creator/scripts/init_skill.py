...
"""
    ...
    (skill_path / "assets/.gitkeep").write_text("", encoding="utf-8")
...
"""
    ...
    print(f"[OK] Skill '{name}' created at {skill_path}")
    print("\nNext steps:")
...
Examples:
  ...
  python init_skill.py my-skill --path ./skills
    ...
    if not all(c.islower() or c == '-' or c.isdigit() for c in args.name):
        print("[ERROR] Skill name must be lowercase with hyphens only")
        exit(1)
