...
name: memory-manager
description: |
  Sistema de memoria persistente POR PROYECTO. Guarda conocimiento en .agent/memory/ dentro del repo,
...
> La memoria vive en `.agent/memory/` DENTRO del repo. Versionada en git. El usuario tiene control total.
...
Crear o actualizar `.agent/settings.json` en la raiz del proyecto:
...
**Por que?** La auto-memory de Claude Code guarda notas en `~/.agent/projects/` (local a la maquina).
Eso significa que:
...
```bash
mkdir -p .agent/memory/{user,feedback,project,reference}
```

### Paso 3: Crear indice MEMORY.md

Crear `.agent/memory/MEMORY.md` con el template base si no existe:
...
Informar: "Memoria persistente activada. Auto-memory de Claude Code deshabilitada. Tu proyecto ahora guarda conocimiento en `.agent/memory/`, versionado con git."
...
### Como consultar
1. Leer `.agent/memory/MEMORY.md` (el indice)
